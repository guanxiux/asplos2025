...
Unfortunately, due to the typically limited computational power and limited and unstable wireless network bandwidth~\cite{yang2022mobile} on the mobile robot which slow down both local computation and naive offloading of computation to GPU servers, the mobile robot often suffers the problem of slow visual model inference that interferes the robotic task performance (e.g., 1.xx seconds in human pose estimation~\cite{kapao} or 2.xx seconds in surrounding occlusion prediction~\cite{agrnav}).
...
% When camera perspective moves, possibly new scenes or changed occlusion cause the consecutive images to differ in a way that cannot be covered by receptive field movement, and such changes happen on a small portion (e.g., 5\%) of the pixels on the input image will cause a larger portion (e.g., 25\%) of pixels on the activations to change, because the receptive fields are often larger than a pixel (e.g., 20X20) and overlap.
% Simply introducing such caching mechanism seems able to accelerate both local computation and computation offloading by either skipping local activation computation or only necessitating the transmission of the recognized movement of receptive fields during computation offloading. But we 
...
% Given a continuous stream of images, they recognize the movement of the receptive fields (represented by motion vectors of the receptive fields) by matching similar pixel blocks on the consecutive input images and interpolate the activations accordingly to form the new activations, skipping the computation of the activations with the local operators and thus accelerating local computation.
